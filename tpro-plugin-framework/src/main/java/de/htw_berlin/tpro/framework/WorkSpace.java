...
class WorkSpace
{
    ...
    public URL[] getClassPath(String compName)
        throws FrameworkException
    {
        String classesDirName = dirName + "/" + compName;
        File tproClassesDir = new File(classesDirName + "/de/htw_berlin/tpro/plugin");
        if(!tproClassesDir.exists())
        {
            throw new FrameworkException("no tpro plugin directory found in " 
                                        + compName);
        }
        if(!tproClassesDir.isDirectory())
        {
            throw new FrameworkException("de/htw_berlin/tpro/plugin is not a directory in " 
                                        + compName);
        }
        URL url;
        try
